...
NOISE_LEVELS=[0.0, 0.001, 0.01, 0.1, 1.]
DEFENSE_ALGOS=["gaussian", "langid", "langunk", "mean"]
DOC_LANGS= ["es", "fr", "de", "en"]
...
def load_embedder_and_tokenizer(name: str, torch_dtype: str, **kwargs):
    ...
    elif name == "sgpt_bloom":
        # for multilingual experiment.
        # no encoder module
        model = transformers.AutoModel.from_pretrained(
            "bigscience/sgpt-bloom-7b1-msmarco", **model_kwargs
        )
        tokenizer = transformers.AutoTokenizer.from_pretrained(
    ...
    #     tokenizer = model.tokenizer
        ...
        tokenizer = transformers.AutoTokenizer.from_pretrained(name)
        tokenizer.pad_token = tokenizer.eos_token # didn't use it for XLM-R models, don't know if it makes a difference
