...
def load_mtg(name) -> datasets.Dataset:
    dataset_path = f"yiyic/{name}"
...
def dataset_from_args(data_args: DataArguments) -> datasets.DatasetDict:
    ...
    # elif data_args.dataset_name == "mtg":
    #     raw_datasets = load_mtg_all()
    elif data_args.dataset_name.startswith("mtg"):
        # with language code, e.g., mtg_zh
        assert "_" in data_args.dataset_name
        name = data_args.dataset_name
        raw_datasets = load_mtg(name)
    elif data_args.dataset_name.startswith("xnli"):
        # it has to be with language codes.
        assert "_" in data_args.dataset_name
        # langs = ["ar", "bg", "de", "el", "en", "es", "fr", "hi", "ru", "sw", "th", "tr",
        # "ur", "vi", "zh"]
        lang = data_args.dataset_name.split("_")[1]
        raw_datasets = load_xnli(lang)
...
def load_mtg_test(lang) -> datasets.Dataset:
    dataset_path = f"yiyic/mtg_{lang}_5m"
    dataset = datasets.load_dataset(dataset_path)["test"]
    return dataset


def load_nq_test() -> datasets.Dataset:
    raw_datasets = load_nq_dpr_corpus()
    dev = raw_datasets["dev"]
    dev = dev.select(range(4000, 7000))
...
def load_standard_val_datasets() -> datasets.DatasetDict:
    ...
    # mtg_en, mtg_fr, mtg_de, mtg_es, nq_en
    d = datasets.load_dataset("yiyic/eval_mtg_clirmatrix_beir")

    # d = load_beir_datasets()
