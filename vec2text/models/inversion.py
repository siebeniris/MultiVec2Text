...
class InversionModel(transformers.PreTrainedModel):
    ...
    def __init__(self, config: InversionConfig):
        ...
        # is this one set the boundary for the token length???? (Comment)
        # determine whether the tokens get a representation.
        ...
        # might need to change the config file for pre-trained models.
    ...
    def _process_embedder_output(
        ...
        if hasattr(outputs, "pooler_output") and (outputs.pooler_output is not None):
            ...
            if self.embeddings_from_layer_n is not None:
                ...
                # added.
                # if self.embedder_model_name == "multilingual_e5_base":
    ...
    def call_embedding_model(
        ...
        if self.defense_algo == "gaussian":
            if self.noise_level > 0:
                print(self.defense_algo, self.noise_level)
                embeddings += self.noise_level * torch.randn(
                    embeddings.shape, device=embeddings.device
                )
        elif self.defense_algo == "langid":
            print(self.defense_algo)
            print(embeddings)
            self.doc_lang_id = lang2ids[self.doc_lang]
            embeddings[:, 0] = self.doc_lang_id
            print("after langid")
            print(embeddings)

        elif self.defense_algo == "langunk":
            print(self.defense_algo)
            embeddings_mean = torch.mean(embeddings, axis=1)
            embeddings_mean = torch.reshape(embeddings_mean, (-1, 1))
            embeddings = torch.sub(embeddings, embeddings_mean)
        elif self.defense_algo == "langmean":
            print(self.defense_algo)
            embeddings_mean = torch.mean(embeddings, axis=1)
            embeddings_mean = torch.reshape(embeddings_mean, (-1, 1))
            embeddings_subtracted = torch.sub(embeddings, embeddings_mean)
            embeddings -= embeddings_subtracted
        else:
            print("no defense")
        return embeddings
