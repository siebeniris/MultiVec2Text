...
def dataset_map_multi_worker(
    ...
    try:
        # got error RuntimeError: Default process group has not been initialized, please make sure to call init_process_group.
        # added, 6th December 2023, Yiyi
        # dist.init_process_group("mapping")
        ...
        if isinstance(num_proc, int) and num_proc > 0:
            ...
            kwargs["num_proc"] = 1  # multi-gpus training without CPUS, LUMI.
        print(f" rank {rank}, world_size {world_size}, kwargs {kwargs}")

    except (RuntimeError, ValueError):
        try:
            num_proc = kwargs.get("num_proc", len(os.sched_getaffinity(0)))
        except AttributeError:  # for MacOS
