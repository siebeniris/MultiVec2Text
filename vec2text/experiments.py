...
print("DATASET_CACHE_PATH", DATASET_CACHE_PATH)
...
def md5_hash_kwargs(**kwargs) -> str:
    ...
    return hashlib.md5(s.encode()).hexdigest()  # hexdigest returns 32 character long digest
...
class Experiment(abc.ABC):
    ...
    def _load_train_dataset_uncached(
        ...
        ###########################################################################
        #### added for test data. 6th Dec, 2023, Yiyi
        ###########################################################################
        # if "test" in tokenized_datasets:
        #     max_test_samples = min(
        #         len(tokenized_datasets["test"]), self.data_args.max_eval_samples
        #     )
        ...
        #         "idx", range(len(tokenized_datasets["test"]))
        #     )
    ...
    def _prepare_val_datasets_dict(
        ...
        for name, dataset in val_datasets_dict.items():
            ...
            print("mtg in model name, lower case")
            val_datasets_dict[name] = val_datasets_dict[name].map(lambda sample: {"text": sample["text"].lower()})
    ...
    def load_train_and_val_datasets(
        ...
        val_dataset_kwargs = {
            "dataset_name": "__".join(
                # ["ag_news", "arxiv", "xsum_doc", "xsum_summ", "wikibio"]  # change this.
                ["mtg_de", "mtg_en", "mtg_de", "mtg_es", "nq_en"]
