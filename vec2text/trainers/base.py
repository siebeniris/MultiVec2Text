...
class BaseTrainer(transformers.Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # added (Yiyi) to save embeddings and decoded sequences..
        self.output_save_dir = self.model.config.output_dir

        # print(f"output results to {self.output_save_dir}")
    ...
    def _get_decoded_sequences(
        ...
        print("trainer get decoded sequences")
    ...
    def eval_generation_metrics(
        ...
        timestamp = int(time())
        outputdir = os.path.join(self.output_save_dir, str(timestamp))
        if not os.path.exists(outputdir):
            os.makedirs(outputdir)

        outputfile = os.path.join(outputdir, f"decoded_sequences.csv")
        ...
        try:
            with torch.no_grad():
                ...
                # self.inversion_trainer.model.noise_level = 0
                ...
                print("shape : ", preds_sample_retokenized.shape )
                preds_sample_retokenized = preds_sample_retokenized[
                : self.args.per_device_eval_batch_size, :
                ]
                print("shape after per_device: ", preds_sample_retokenized.shape)
                ...
                outfile_emb_preds = os.path.join(outputdir, f"embeddings_preds.npy")
                outfile_emb_labels = os.path.join(outputdir, f"embeddings_labels.npy")
