...
class CorrectorEncoderModel(transformers.PreTrainedModel):
    ...
    def __init__(
        ...
        if config.embedder_model_api:
            ...
            embedder_dim = config.embedder_dim  # change this to be fed by config file. (Yiyi)
        ...
        self.encoder_hidden_dim = self.encoder_decoder.config.hidden_size  # that's for t5-base.
    ...
    def get_encoder_embedding(
        ...
        # print("batch size:", batch_size, "embedding shape", embedding.shape, "embedder dim", self.embedder_dim, "hypothesis embedding shape:", hypothesis_embedding.shape )
        # set_trace()
